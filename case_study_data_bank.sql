...
*/
select r.region_name  as 'Region Name', count(distinct cn.node_id) as 'Number of Nodes'
...
*/
select r.region_name  as 'Region Name', count( distinct cn.customer_id) as 'Number of Customers'
from customer_nodes as cn
inner join regions as r on cn.region_id = r.region_id
group by r.region_name
order by count(cn.customer_id) desc;

--------------------------------------------------------------------------------------------
/*
4. How many days on average are customers reallocated to a different node?
*/
select round(avg(datediff(end_date, start_date)), 0) as 'Average days'
from customer_nodes
...
with days as(
			...
			select * from customer_nodes
            where region_id = 3;*/
			select cn.region_id, r.region_name, datediff(cn.end_date, cn.start_date) as reallocation_days
            from customer_nodes as cn
            inner join regions as r using(region_id)
            where cn.end_date <> '9999-12-31' -- and region_id = 3
),
ranked_data as(
				select region_name, reallocation_days,
                row_number() over(partition by region_name order by reallocation_days) as row_num,
...
percentiles as( 
            ...
            max(case
					when row_num = floor(0.5 * total_count) + 1 then reallocation_days
                    end
				) as 'Median(in days)',
			max(case
					when row_num = floor(0.8 * total_count) + 1 then reallocation_days
                    end
				) as 'Percentile_80(in days)',
			max(case
					when row_num = floor(0.95 * total_count) + 1 then reallocation_days
                    end
				) as 'Percentile_95(in days)'
			from ranked_data
...
*/
with cte as(
			select customer_id, count(txn_date) txn_count, count(distinct(month(txn_date))) dis_month, 
...
select customer_id, round((txn_count / dis_month), 0) 'average total historical deposit counts', round((total_amount / dis_month), 2) as avg_amount
from cte
group by customer_id
order by customer_id;
...
from customer_transactions
where count(txn_type = 'deposit')>1 and (count(txn_type = 'purchase') = 1 or count(txn_type = 'withdrawal') = 1)
...
balance_growth as (
					select customer_id, month_no, closing_balance,
					lag(closing_balance) over (partition by customer_id order by month_no) as previous_balance,
					case 
						when lag(closing_balance) over (partition by customer_id order by month_no) > 0 
						then round(((closing_balance - lag(closing_balance) over (partition by customer_id order by month_no)) / lag(closing_balance) over (partition by customer_id order by month_no)), 2) * 100
						else 0
					end as growth_percentage
					from monthly_balances
),
customers_with_growth as (
							select distinct customer_id, growth_percentage
							from balance_growth
							where growth_percentage > 5
),
total_customers as (
...
select 
round(((select count(*) from customers_with_growth) / (select total_customer_count from total_customers)), 2) * 100.0  as percentage_of_customers_with_growth;
...
select customer_id, min(running_balance) as min_transaction, max(running_balance) as max_transaction,
